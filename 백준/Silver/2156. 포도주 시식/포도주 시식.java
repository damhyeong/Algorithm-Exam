...
2 초	128 MB	144892	49564	35832	32.681%
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		if(n == 2)
			...
			cost[1] = arr[1]; cost[2] = arr[1] + arr[2];
			for(int i = 3; i < n + 1; i++) {
				int result1 = arr[i] + cost[i - 2];
				int result2 = arr[i] + arr[i - 1] + cost[i - 3];
				int result3 = cost[i - 1];
				cost[i] = Math.max(result1, Math.max(result2, result3));
			}
		}
		
		int max = Integer.MIN_VALUE;
		for(int i = 0; i < n + 1; i++)
			max = max < cost[i] ? cost[i] : max;
