...
1 초	128 MB	25734	8785	7372	34.056%
...
자칫 간식을 못 받게 될지도 모른다는 위기감을 느낀 승환이는 자신의 컴퓨터 알고리즘적 지식을 활용해 
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		while(!queue.isEmpty() || !stack.isEmpty()) {
			...
			if(wait1 != null && wait1 == index) {
				queue.poll();
				index++;
			} else if(wait2 != null && wait2 == index) {
				stack.pop();
				index++;
			} else {
				if(queue.isEmpty()) {
		...
		System.out.println(isSequenced ? "Nice" : "Sad");
	}
