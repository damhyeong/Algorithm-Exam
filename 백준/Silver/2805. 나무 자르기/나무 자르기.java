...
1 초	256 MB	208439	61747	38330	26.300%
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		long result = getMinTreeLen(trees, M);
		
		System.out.println(result);
	}
	
	// 관심사 분리 - 최소로 나무를 자를 수 있는 높이의 최댓값 구하
	public static long getMinTreeLen(long[] trees, long M) {
		return getTreeLen(trees, M, 0, Integer.MAX_VALUE);
	}
	
	public static long getTreeLen(long[] trees, long M, long sLen, long eLen) {
		if(sLen > eLen)
			return -1;
		
		long mid = (sLen + eLen) / 2;
		
		long treeLen = cutTreeLen(trees, mid);
		
		if(treeLen < M)
			return getTreeLen(trees, M, sLen, mid - 1);
		else {
			long temp = getTreeLen(trees, M, mid + 1, eLen);
			if(temp == -1)
				return mid;
			else
				return temp;
	...
	// return : 잘려질 나무의 총 길이의 합을 반환. 위 메서드의 M 과 비교하게 됨.
	public static long cutTreeLen(long[] trees, long height) {
		long sum = 0;
