...
2 초	512 MB	69812	27353	21298	38.690%
...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		long[] loadLen = new long[cityNum];
		long[] cityCosts = new long[cityNum];
		
		StringTokenizer st = new StringTokenizer(br.readLine());
		loadLen[0] = 0;
		for(int i = 1; i < loadLen.length; i++) 
			loadLen[i] = Long.parseLong(st.nextToken()) + loadLen[i - 1];
		
		st = new StringTokenizer(br.readLine());
		for(int i = 0; i < cityCosts.length; i++)
			cityCosts[i] = Long.parseLong(st.nextToken());
		
		int currIdx = 0;
		long minCost = cityCosts[0];
		long totalCost = 0;
		
		for(int i = 1; i < cityCosts.length; i++) {
			if(minCost >= cityCosts[i]) {
				totalCost += minCost * (loadLen[i] - loadLen[currIdx]);
				currIdx = i;
				minCost = cityCosts[i];
			}
		}
		totalCost += minCost * ((loadLen[cityCosts.length - 1]) - loadLen[currIdx]);
